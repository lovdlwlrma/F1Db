...
export const StandingsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (cached) {
          const parsed = JSON.parse(cached) as { timestamp: number; data: SeasonData[] };
        ...
        const seasonData = await StandingsService.getDriverStanding(currentYear);
        setData(seasonData);
  ...
  if (!data || !Array.isArray(data) || data.length === 0 || !data[0]) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black flex items-center justify-center">
        <div className="text-center">
          <div className="text-yellow-500 text-2xl font-bold mb-4">⚠️ No Data</div>
          <div className="text-white text-lg">No season data found for {currentYear}</div>
          <button
            onClick={() => window.location.reload()}

...
const AnalyticsPage: React.FC = () => {
  ...
  const [lapRankings, setLapRankings] = useState<SessionLapRankingResponse | null>(null);
    ...
    const fetchRaceSession = async () => {
      try {
        const sessions =
          ...
          (await withRetry(() => AnalyticsService.getRaceSession(grandPrix.meeting_key), 3, 1500));
        setCache((prev) => ({
          ...prev,
    ...
    const fetchLapRankings = async () => {
      try {
        const res =
          ...
          (await withRetry(() => AnalyticsService.getLapRankings(session.session_key), 3, 1500));
        setCache((prev) => ({
          ...prev,
    ...
    const fetchDrivers = async () => {
      try {
        const res =
          ...
          (await withRetry(() => AnalyticsService.getDrivers(session.session_key), 3, 1500));
        setCache((prev) => ({
          ...prev,
    ...
    const fetchStints = async () => {
      try {
        const res =
          ...
          (await withRetry(() => AnalyticsService.getStints(session.session_key), 3, 1500));
        setCache((prev) => ({
          ...prev,
    ...
    const fetchResults = async () => {
      try {
        const res =
          ...
          (await withRetry(() => AnalyticsService.getResult(session.session_key), 3, 1500));
        setCache((prev) => ({
          ...prev,
    ...
    const fetchRaceControls = async () => {
      try {
        const res =
          ...
          (await withRetry(() => AnalyticsService.getRaceControl(session.session_key), 3, 1500));
        setCache((prev) => ({
          ...prev,
  ...
  const isChartsReady = isTableReady && isLapRankingReady && isStintReady;
    ...
    <AnalyticsLayout>
      <div className="space-y-6">
        ...
        <SelectBar
          ...
          setGrandPrix={(name) => {
            const meeting =
              cache.meetings[year]?.find((m) => m.meeting_name === name) ?? null;
            resetData();
        ...
        {!isChartsReady ? (
          <>
            <div className="bg-gray-800 animate-pulse rounded-2xl" style={{ height: "44rem" }} />
            <div className="bg-gray-800 animate-pulse rounded" style={{ height: "40rem" }} />
            <div className="bg-gray-800 animate-pulse rounded" style={{ height: "40rem" }} />
            <div className="bg-gray-800 animate-pulse rounded-2xl" style={{ height: "40rem" }} />
          </>
        ) : (
          <>
            <RaceStatisticsTable

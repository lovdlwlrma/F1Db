...
import { Meeting, Session, Driver, LapData, TelemetryData } from "@/types/telemetry";
...
const TelemetryPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchLaps = async () => {
      ...
      setLoading(true);
      try {
        const lapData = await TelemetryService.getLaps(session.session_key, selectedDriver1.driver_number);
        setLaps1(lapData);

        const valid = lapData.filter(l => l.lap_duration !== null && l.lap_number !== 1 && l.lap_number !== lapData.length);
        const fastestLap = valid.length > 0 ? valid.reduce((p, c) => (p.lap_duration! < c.lap_duration! ? p : c), valid[0]) : null;
  ...
  useEffect(() => {
    ...
    const fetchLaps = async () => {
      ...
      setLoading(true);
      try {
        const lapData = await TelemetryService.getLaps(session.session_key, selectedDriver2.driver_number);
        setLaps2(lapData);
  ...
  useEffect(() => {
    ...
    const fetchTelemetry = async () => {
      ...
      setLoading(true);
      try {
        const carData = await TelemetryService.getTelemetry(session.session_key, selectedDriver1.driver_number, selectedLap1.lap_number);
  ...
  useEffect(() => {
    ...
    const fetchTelemetry = async () => {
      ...
      setLoading(true);
      try {
        const carData = await TelemetryService.getTelemetry(session.session_key, selectedDriver2.driver_number, selectedLap2.lap_number);
  ...
  return (
    <div className="p-4">
      <SelectBar
        ...
        country={grandPrix?.meeting_name ?? ""} setCountry={(name) => {
          const meeting = meetings.find(m => m.meeting_name === name) ?? null;
          setGrandPrix(meeting);
        }}
        sessionName={session?.session_name ?? ""} setSessionName={(name) => {
        ...
        selectedDriver1={selectedDriver1} setSelectedDriver1={setSelectedDriver1}
        selectedDriver2={selectedDriver2} setSelectedDriver2={setSelectedDriver2}
        laps1={laps1} selectedLap1={selectedLap1} setSelectedLap1={setSelectedLap1}
        laps2={laps2} selectedLap2={selectedLap2} setSelectedLap2={setSelectedLap2}
      />

      {loading ? (
        <div className="grid grid-cols-2 gap-6">
          {Array.from({ length: 6 }).map((_, idx) => (
            <div key={idx} className="h-96 bg-gray-800 animate-pulse rounded"></div>
          ))}
        </div>
      ) : (
        <TelemetryChart

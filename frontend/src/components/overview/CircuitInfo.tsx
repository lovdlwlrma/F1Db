...
const CircuitInfo: React.FC<CircuitInfoProps> = ({ nextRace, loading }) => {
  ...
  if (circuit?.info) {
    circuit.info.forEach(item => {
      circuitInfoMap[item.key] = item.value + (item.annotation ? ` ${item.annotation}` : '');
    });
  }
  ...
  const infoCards = [
    ...
    { label: "Number of Laps", value: circuitInfoMap["Number of Laps"], icon: Hash },
    { label: "Circuit Length", value: circuitInfoMap["Circuit Length"], icon: AudioWaveform },
    { label: "Race Distance", value: circuitInfoMap["Race Distance"], icon: Map },
    { label: "Fastest Lap Time", value: circuitInfoMap["Lap Record"], icon: Clock },
                ...
                >
                    ...
                    <div className="flex flex-col ml-3 text-left">
                        ...
                        {isFastestLap ? (
                            ...
                            {lapParts[1] && <span className="text-gray-300 text-sm">{lapParts[1]}</span>}
                            </div>
                        ) : (

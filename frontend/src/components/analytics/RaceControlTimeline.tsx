...
const RaceControlTimeline: React.FC<RaceControlTimelineProps> = ({
  ...
  const getCategoryColor = (
    ...
    if (category.toLowerCase() === "flag" && flag) {
      switch (flag.toLowerCase()) {
        ...
        case "checkered":
          return "bg-gradient-to-r from-gray-800 to-white text-gray-800 border-gray-400";
        default:
    ...
    switch (category.toLowerCase()) {
      ...
      default:
        return "bg-indigo-500 text-indigo-100 border-indigo-400";
    }
  };
    ...
    return (
      ...
      >
        <div className="text-center text-gray-400">
          ...
          <p className="text-sm">
            Race control data will appear here during the session
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6 relative">
      ...
      <div className="absolute top-6 right-6">
        <div className="grid grid-cols-5 gap-3 text-center mr-2">
          {["Flag", "SafetyCar", "DRS", "FIA", "Other"].map((category) => {
            const count = sortedControls.filter((c) =>
              category === "FIA"
                ? isInvestigation(c.message)
                : c.category.toLowerCase() === category.toLowerCase(),
            ).length;
            return (
              <div
                key={category}
                className="bg-gray-800 rounded-lg px-3 py-2 min-w-[90px]"
              >
                <div className="text-base font-bold text-white">{count}</div>
                <div className="text-[10px] text-gray-400">
                  {category === "SafetyCar" ? "Safety Car" : category}
                </div>
              </div>
            );
          })}
      ...
      <div
        className="relative overflow-y-auto scrollbar-thin scrollbar-track-gray-800 scrollbar-thumb-gray-600 pr-2"
          ...
          {sortedControls.map((control) => (
            <div className="relative flex items-start gap-9 group">
              ...
              <div className="relative z-10 flex-shrink-0 self-center ml-2">
                <div
                  className={`w-10 h-10 rounded-full border-2 flex items-center justify-center ${getCategoryColor(control.category, control.flag, control.message)} transition-transform duration-200 group-hover:scale-110 shadow-md`}
                >
              ...
              <div className="flex-1 min-w-0 bg-gray-800/80 rounded-lg border border-gray-700 px-4 py-3 group-hover:border-gray-600 group-hover:bg-gray-750 transition-all duration-200 shadow-sm">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span
                      className={`px-2 py-0.5 rounded-md text-xs font-semibold border shadow-sm ${getCategoryColor(control.category, control.flag, control.message)}`}
                    >
                      {isInvestigation(control.message)
                        ? "FIA"
                        : control.flag
                          ? `${control.category} - ${control.flag}`
                          : control.category}
                    </span>

                    {(control.driver_number ||
                      control.lap_number > 0 ||
                      control.sector) && (
                      <div className="flex items-center gap-2 text-xs text-gray-400">
                        ...
                        {control.sector && <span>Sector {control.sector}</span>}
                      </div>
                    )}
                  </div>

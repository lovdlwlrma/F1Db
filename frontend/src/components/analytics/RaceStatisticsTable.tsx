...
import { Driver, Result, SessionLapRankingResponse, TeamStats } from "@/types/analytics";
...
export default function RaceStatisticsTable({ drivers, results, lapRankings }: Props) {
  ...
  // 計算車手積分
  const getDriverPoints = (position: number | undefined, dnf: boolean, dns: boolean, dsq: boolean): number => {
  ...
  const teamStats = useMemo(() => {
    ...
    validResults.forEach(result => {
      ...
      const points = getDriverPoints(result.position, result.dnf, result.dns, result.dsq);
  ...
  const podiumDrivers = useMemo(() => {
    return validResults
      .filter(r => r.position !== undefined && r.position <= 3 && !r.dnf && !r.dns && !r.dsq)
  ...
  const raceStats = useMemo(() => {
    const finishers = validResults.filter(r => !r.dnf && !r.dns && !r.dsq).length;
    ...
    return {
      ...
      totalDrivers,
      validDrivers,
      completionRate: validDrivers > 0 ? Math.round((finishers / validDrivers) * 100) : 0
    };
  }, [results, validResults, lapRankings]);

  const getStatusText = (result: { dnf: boolean; dns: boolean; dsq: boolean }) => {
  ...
  const getStatusColor = (result: { dnf: boolean; dns: boolean; dsq: boolean }) => {
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-lg font-bold text-white">P{result.position}</div>
                      <div className="text-sm text-gray-400">{result.points} points</div>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="space-y-4 mt-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-3 text-center">
                <div className="text-xl font-bold text-blue-400">{raceStats.totalLaps}</div>
              ...
              <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-3 text-center">
                <div className="text-xl font-bold text-purple-400">{raceStats.completionRate}%</div>
                <div className="text-xs text-gray-300">Completion Rate</div>
              </div>
              
              <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-3 text-center">
                <div className="text-xl font-bold text-green-400">{raceStats.finishers}</div>
                <div className="text-xs text-gray-300">Finishers</div>
              </div>
              
              <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-3 text-center">
                <div className="text-xl font-bold text-red-400">{raceStats.dnfCount}</div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {raceStats.dnsCount > 0 && (
                  <div className="bg-gray-500/10 border border-gray-500/30 rounded-xl p-3 text-center">
                    <div className="text-xl font-bold text-gray-400">{raceStats.dnsCount}</div>
                ...
                {raceStats.dsqCount > 0 && (
                  <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-3 text-center">
                    <div className="text-xl font-bold text-orange-400">{raceStats.dsqCount}</div>
            ...
            {teamStats.slice(0, 5).map((team, index) => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-xl font-bold text-white">{team.points} points</div>
                </div>
                
                <div className="space-y-1">
                  {team.drivers.map((driver, driverIndex) => (
                    <div key={driverIndex} className="flex justify-between text-sm">
                      <span className="text-gray-300">{driver.name}</span>
                      <span className="text-gray-400">
                        {driver.dnf && ' DNF'} {driver.dns && ' DNS'} {driver.dsq && ' DSQ'}
                        {driver.position !== undefined && driver.position !== null ? `P${driver.position}` : ''} ({driver.points} points)
                      </span>
                    </div>
                  ))}
                </div>
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="bg-gray-800/60 border border-gray-600/50 rounded-xl p-4 flex-1">
            ...
            .map((result) => {
                ...
                return (
                <div key={result.driver_number} className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                    <span className="w-6 text-center text-gray-400">
                        {result.position != null ? `P${result.position}` : `${result.dnf ? '' : result.dns ? '' : result.dsq ? '' : 'Finished'}`}
                    </span>
                    <span className="text-m font-medium px-2" style={{ color: `#${driver.team_colour}` }}>
                        {driver.full_name}
                    </span>
                    </div>
                    <div className="flex items-center gap-3">
                    <span className="text-gray-300">{points} points</span>
                    <span className={`px-2 py-1 rounded text-xs ${getStatusColor(result)} w-16`}>

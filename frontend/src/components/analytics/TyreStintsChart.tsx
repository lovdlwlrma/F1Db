...
import { Driver, Stints, Result } from "@/types/analytics";
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
interface Props {
  stints?: Record<number, Stints[]> | null;
  drivers?: Driver[];
  results?: Result[];
  height?: number;
}
...
export default function TyreStintsChart({ stints, drivers = [], results = [], height = 300 }: Props) {
  ...
  const chartData = useMemo(() => {
    ...
    const maxLap = Math.max(...Object.values(stints).flat().map(s => s.lap_end));
    const maxStints = Math.max(...Object.values(stints).map(driverStints => driverStints.length));
    ...
    for (let stintIndex = 0; stintIndex < maxStints; stintIndex++) {
      ...
      const backgroundColors = driverNumbers.map(driverNumber => {
        ...
        const hex = tyreColors[stint.compound as keyof typeof tyreColors] || tyreColors.DNF;
      ...
      tooltip: {
        ...
        callbacks: {
          label: ctx => {
            ...
            // driverNumber
            const driverNumbers = Object.keys(stints).map(Number).sort((a, b) => {
              const pa = resultMap[a] ?? Infinity;
              const pb = resultMap[b] ?? Infinity;
              if (pa !== pb) return pa - pb;
            ...
            const driverStints = (stints[driverNumber] || []).sort((a,b) => a.lap_start - b.lap_start);
          
            const laps = ctx.raw as number;
          
            // 找出對應 laps 的 stint
            const stint = driverStints.find(s => (s.lap_end - s.lap_start + 1) === laps);

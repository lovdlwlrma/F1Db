...
const SelectBar: React.FC<Props> = ({ year, setYear, grandPrix, setGrandPrix, grandPrixList }) => {
        ...
        <SelectContent className={selectContentClass}>
          <SelectGroup>
            {[2023, 2024, 2025].map((y) => (
              <SelectItem key={`year-${y}`} value={y.toString()} className={selectItemClass}>
                {y}
              </SelectItem>
            ))}
          </SelectGroup>
        ...
        <SelectContent className={selectContentClass}>
          <SelectGroup>
            {grandPrixList.map((name) => (
              <SelectItem key={`gp-${name}`} value={name} className={selectItemClass}>
                {name}
              </SelectItem>
            ))}
          </SelectGroup>
